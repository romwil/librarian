"""Dedicated SQLite write serializer (Projectionist-style).
...
reads do not regress. Callers keep the existing sync ``Database`` API; clear /
ingest / Review Apply route mutations through ``run_write`` so they do not fight
each other for the same SQLite writer lock.
"""
...
# Homelab default: absorb a burst of clear/ingest/poller writes without unbounded
# memory growth. Blocked put() is the backpressure signal.
