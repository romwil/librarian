...
def _mail_settings(settings: Any) -> Any:
    """Accept Settings, MailSettings, or a mapping-like mail object."""
    from librarian.config import MailSettings

    if isinstance(settings, MailSettings):
        return settings
    mail = getattr(settings, "mail", None)
    if isinstance(mail, MailSettings):
        return mail
    if isinstance(settings, dict):
        return MailSettings.from_mapping(settings)
    return MailSettings()


def mail_configured(settings: Any) -> bool:
    """Return True when outbound mail can be attempted."""
    mail = _mail_settings(settings)
    if not mail.enabled:
        return False
...
def _apply_subject_prefix(mail: Any, subject: str) -> str:
    cleaned = str(subject or "").strip() or "Librarian"
...
def _html_body(mail: Any, *, body_text: str, body_html: Optional[str] = None) -> str:
...
def _send_smtp(mail: Any, *, to_email: str, subject: str, text: str, html: str) -> MailSendResult:
...
def _send_resend(mail: Any, *, to_email: str, subject: str, text: str, html: str) -> MailSendResult:
...
def send_mail(
    ...
    *,
    ...
    if not mail_configured(mail):
        raise MailSendError("Mail is not configured (enable SMTP or Resend in Settings → Mail)")
