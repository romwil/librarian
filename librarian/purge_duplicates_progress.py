...
def progress_path(data_dir: Path) -> Path:
    return Path(data_dir) / "purge_duplicates_progress.json"
...
def append_purge_duplicates_log(data_dir: Path, line: str) -> Dict[str, Any]:
    ...
    if not text:
        return read_purge_duplicates_progress(data_dir)
    with _lock:
        current = _read_unlocked(data_dir)
    ...
    payload.update(
        {
            ...
            "logs": ["Started purging redundant Review slips."],
        }
    )
    return write_purge_duplicates_progress(data_dir, payload)
    ...
    *,
    ...
    with _lock:
        ...
        if error:
            ...
            for key in ("purged", "shelf_twins", "slip_twins", "kept", "failed", "done", "total"):
                if key in summary:
...
def is_purge_duplicates_running(data_dir: Path) -> bool:
    return str(read_purge_duplicates_progress(data_dir).get("status") or "") == "running"
